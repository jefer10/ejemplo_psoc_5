...
#include "display.h"
#include <display_DMA_Com_dma.h>
#include <display_DMA_Seg_dma.h>
#if (display_BRIGHTNESS_EN == display_ENABLED)
    #include <display_DMA_BC_dma.h>
#endif /* (display_BRIGHTNESS_EN == display_ENABLED) */

/* Function prototypes */
static void display_DmaInit(void) ;
static void display_DmaEnable(void) ;
static void display_DmaDisable(void) ;

/* Used to hold the common line output sequence */
#if(display_COM_DRIVE == display_HIGH)
    static uint8 display_comBuffer[16] = {0x00u, 0x01u, 0x00u, 0x02u, 0x00u, 0x04u, 0x00u, 0x08u,
                                                    0x00u, 0x10u, 0x00u, 0x20u, 0x00u, 0x40u, 0x00u, 0x80u};
#else
    static uint8 display_comBuffer[16] = {0xFFu, 0xFEu, 0xFFu, 0xFDu, 0xFFu, 0xFBu, 0xFFu, 0xF7u,
                                                    0xFFu, 0xEFu, 0xFFu, 0xDFu, 0xFFu, 0xBFu, 0xFFu, 0x7Fu};
#endif /* (display_COM_DRIVE == display_HIGH) */

/* Buffers used to hold the display(segment) values to be transferred to the control registers */
uint8 display_segLBuffer[display_NUM_COM];
#if (display_NUM_SEG > 8u) 
    uint8 display_segMBuffer[display_NUM_COM];
#endif /* (display_NUM_SEG > 8u) */
#if (display_NUM_SEG > 16u) 
    uint8 display_segHBuffer[display_NUM_COM];
#endif /* (display_NUM_SEG > 16u) */

#if (display_BRIGHTNESS_EN == display_ENABLED)
    /* The brightness register used to hold the brightness levels for each common */
    static uint8 display_brightnessReg[display_NUM_COM];
#endif /* (display_BRIGHTNESS_EN == display_ENABLED) */

/* DMA channel and TD declaractions */
static uint8 display_dmaComChannel;
static uint8 display_dmaComTd[display_TWO_NUM_COM];

static uint8 display_dmaSegChannel;
static uint8 display_dmaSegLTd[display_NUM_COM];

#if (display_BRIGHTNESS_EN == display_ENABLED)
    static uint8 display_dmaBCChannel;
    static uint8 display_dmaBCTd;
#endif /* (display_BRIGHTNESS_EN == display_ENABLED) */

uint8 display_initVar = 0u;
uint8 display_enableVar = 0u;


/*******************************************************************************
* Function Name: display_DmaInit
********************************************************************************
*
* Summary: 
...
static void display_DmaInit(void) 
{
    ...
    #if (display_NUM_SEG > 8u)
        static uint8 display_dmaSegMTd[display_NUM_COM];
    #endif /* (display_NUM_SEG > 8u) */
    #if (display_NUM_SEG > 16u)
        static uint8 display_dmaSegHTd[display_NUM_COM];
    #endif /* (display_NUM_SEG > 16u) */

    /************************* DMA configuration for the Commons *************************/
    /* Common DMA channel */
    display_dmaComChannel = display_DMA_Com_DmaInitialize(1u, 1u, 
                                        HI16(display_SRAM_BASE), HI16(display_CONTROL_REG_BASE));

    /* Check if the channel is valid */                                                                        
    if(display_dmaComChannel != DMA_INVALID_CHANNEL)
    {
        /* Allocate the common TDs */
        for(i = 0u; i < display_TWO_NUM_COM; i++)
        {
            display_dmaComTd[i] = CyDmaTdAllocate();
        }
        
        /* Set configuration for the Common TDs */
        for(i = 0u;  i < (display_TWO_NUM_COM - 1u); i++)
        {
            /* Separate configurations for odd and even TDs */
            if((uint8)(i & 1u) == 0u)
            {
                /* Set configuration for the Common TDs */
                (void) CyDmaTdSetConfiguration(display_dmaComTd[i], 1u, 
                                                display_dmaComTd[i+1u],
                                                TD_INC_SRC_ADR | display_DMA_Com__TD_TERMOUT_EN);
            }
            else
            {
                /* Set configuration for the Common TDs */
                (void) CyDmaTdSetConfiguration(display_dmaComTd[i], 1u,
                                                display_dmaComTd[i+1u], TD_INC_SRC_ADR);
            }
        }
        /* Set the wraparound Common TD configuration */
        (void) CyDmaTdSetConfiguration(display_dmaComTd[display_TWO_NUM_COM - 1u], 
                                        1u, display_dmaComTd[0u], TD_INC_SRC_ADR);
        
        /* Set the lower addresses of the common TDs */
        for(i = 0u; i < display_TWO_NUM_COM; i++)
        {
            (void) CyDmaTdSetAddress(display_dmaComTd[i],
                                        LO16((uint32) &display_comBuffer[i]),
                                        LO16((uint32) display_COM_CONTROL_PTR));
        }    
    }
    ...
    display_dmaSegChannel = display_DMA_Seg_DmaInitialize(1u, 1u, 
                                        HI16(display_SRAM_BASE), HI16(display_CONTROL_REG_BASE));
    
    /* Check if the segment channel is valid */
    if(display_dmaSegChannel != DMA_INVALID_CHANNEL)
    {
        /* Allocate separate Tds for segments to implement the chain */
        for(i = 0u; i < display_NUM_COM; i++)
        {
            display_dmaSegLTd[i] = CyDmaTdAllocate();
            #if (display_NUM_SEG > 8u)
                display_dmaSegMTd[i] = CyDmaTdAllocate();
            #endif /* (display_NUM_SEG > 8u) */
            #if (display_NUM_SEG > 16u)
                display_dmaSegHTd[i] = CyDmaTdAllocate();
            #endif /* (display_NUM_SEG > 16u) */
        }
        
        /* Set the configuration for the Segment TDs */
        for(i = 0u; i < (display_NUM_COM - 1u); i++)
        {
            #if (display_NUM_SEG <= 8u)
                (void) CyDmaTdSetConfiguration(display_dmaSegLTd[i], 1u, display_dmaSegLTd[i+1u],
                                                TD_INC_SRC_ADR | display_DMA_Seg__TD_TERMOUT_EN);
            #else    
                (void) CyDmaTdSetConfiguration(display_dmaSegLTd[i], 1u, display_dmaSegMTd[i],
                                                TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);

                #if (display_NUM_SEG <= 16u)
                    (void) CyDmaTdSetConfiguration(display_dmaSegMTd[i], 1u, display_dmaSegLTd[i+1u],
                                                    TD_INC_SRC_ADR | display_DMA_Seg__TD_TERMOUT_EN);
                #else    
                    (void) CyDmaTdSetConfiguration(display_dmaSegMTd[i], 1u, display_dmaSegHTd[i],
                                                    TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
                    (void) CyDmaTdSetConfiguration(display_dmaSegHTd[i], 1u, display_dmaSegLTd[i+1u],
                                                    TD_INC_SRC_ADR | display_DMA_Seg__TD_TERMOUT_EN);
                #endif /* display_NUM_SEG <= 16u */
            #endif /* display_NUM_SEG <= 8u */
        }
        ...
        #if (display_NUM_SEG <= 8u)
            (void) CyDmaTdSetConfiguration(display_dmaSegLTd[display_NUM_COM - 1u],
                                            1u, display_dmaSegLTd[0u], 
                                            TD_INC_SRC_ADR | display_DMA_Seg__TD_TERMOUT_EN);
        #else    
            (void) CyDmaTdSetConfiguration(display_dmaSegLTd[display_NUM_COM - 1u],
                                            1u, display_dmaSegMTd[display_NUM_COM - 1u],
                                            TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
            #if (display_NUM_SEG <= 16u)
                (void) CyDmaTdSetConfiguration(display_dmaSegMTd[display_NUM_COM - 1u],
                                                1u, display_dmaSegLTd[0u], 
                                                TD_INC_SRC_ADR | display_DMA_Seg__TD_TERMOUT_EN);
            #else    
                (void) CyDmaTdSetConfiguration(display_dmaSegMTd[display_NUM_COM - 1u],
                                                1u, display_dmaSegHTd[display_NUM_COM - 1u], 
                                                TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
                (void) CyDmaTdSetConfiguration(display_dmaSegHTd[display_NUM_COM - 1u],
                                                1u, display_dmaSegLTd[0u], 
                                                TD_INC_SRC_ADR | display_DMA_Seg__TD_TERMOUT_EN);
            #endif /* (display_NUM_SEG <= 16u) */
        #endif /* (display_NUM_SEG <= 8u) */
        ...
        for(i = 0u; i < display_NUM_COM; i++)
        {
            (void) CyDmaTdSetAddress(display_dmaSegLTd[i],
                                            LO16((uint32) &display_segLBuffer[i]),
                                            LO16((uint32) display_SEG_L_CONTROL_PTR));
            #if (display_NUM_SEG > 8u)
                (void) CyDmaTdSetAddress(display_dmaSegMTd[i],
                                            LO16((uint32) &display_segMBuffer[i]),
                                            LO16((uint32) display_SEG_M_CONTROL_PTR));
            #endif /* (display_NUM_SEG > 8u) */
            #if (display_NUM_SEG > 16u)
                (void) CyDmaTdSetAddress(display_dmaSegHTd[i], 
                                            LO16((uint32) &display_segHBuffer[i]),
                                            LO16((uint32) display_SEG_H_CONTROL_PTR));
            #endif /* (display_NUM_SEG > 16u) */
        }
    }
    ...
    /******************** DMA configuration for Brightness control *********************/
        ...
        display_dmaBCChannel = display_DMA_BC_DmaInitialize(1u, 1u, 
                                        HI16(display_SRAM_BASE), HI16(display_CONTROL_REG_BASE));
            
        /* Check if the channels are valid */
        if(display_dmaBCChannel != DMA_INVALID_CHANNEL)
        {
            display_dmaBCTd = CyDmaTdAllocate();
            
            /* Set the TD configuration for the brightness DMA */
            (void) CyDmaTdSetConfiguration(display_dmaBCTd, display_NUM_COM,
                                        display_dmaBCTd, TD_INC_SRC_ADR);
            
            /* Lower address for the brightness Td */
            (void) CyDmaTdSetAddress(display_dmaBCTd, LO16((uint32) &display_brightnessReg[0u]),
                                                               LO16((uint32)display_COMPARE_PTR));
        }
    #endif /* (display_BRIGHTNESS_EN == display_ENABLED) */
}
...
/*******************************************************************************
* Function Name: display_DmaEnable
********************************************************************************
*
* Summary: 
...
static void display_DmaEnable(void) 
{
    ...
    (void) CyDmaChSetInitialTd(display_dmaComChannel, display_dmaComTd[0u]);
    (void) CyDmaChSetInitialTd(display_dmaSegChannel, display_dmaSegLTd[0u]);
    #if (display_BRIGHTNESS_EN == display_ENABLED)
        (void) CyDmaChSetInitialTd(display_dmaBCChannel, display_dmaBCTd);
    #endif /* (display_BRIGHTNESS_EN == display_ENABLED) */
    
    /* Enable all channels */
    (void) CyDmaChEnable(display_dmaComChannel, 1u);
    (void) CyDmaChEnable(display_dmaSegChannel, 1u);
    #if (display_BRIGHTNESS_EN == display_ENABLED)
        (void) CyDmaChEnable(display_dmaBCChannel, 1u);
    #endif /* (display_BRIGHTNESS_EN == display_ENABLED) */
}


/*******************************************************************************
* Function Name: display_DmaDisable
********************************************************************************
*
* Summary: 
...
static void display_DmaDisable(void) 
{
    uint8 i;
    uint8 channel;
    uint8 dmaChannelList[display_CHANNEL_LIMIT];
    
    dmaChannelList[0u] = display_dmaComChannel;
    dmaChannelList[1u] = display_dmaSegChannel;
    #if (display_BRIGHTNESS_EN == display_ENABLED)
        dmaChannelList[2u] = display_dmaBCChannel;
    #endif /* (display_BRIGHTNESS_EN == display_ENABLED) */

    for(i = 0u; i < display_CHANNEL_LIMIT; i++)
    {
        channel = dmaChannelList[i];
        (void) CyDmaChDisable(channel);
        
        display_Com_Driver_Write(display_CLEAR);
...
{
    static uint8 display_dmaInitDone = 0u;
    #if (display_BRIGHTNESS_EN == display_ENABLED)
        uint8 i;
    #endif /* (display_BRIGHTNESS_EN == display_ENABLED) */
    
    /* Start fresh by clearing the display */
    display_ClearDisplayAll();
    
    /* Initialize the DMAs */
    if(0u == display_dmaInitDone)
    {
        display_DmaInit();
        display_dmaInitDone = 1u;
    }
    ...
    /* Set the brightness to full */
        ...
        for(i = 0u; i<(display_NUM_COM); i++)
        {
            display_brightnessReg[i] = display_INIT_BRIGHT;
        }
    #endif /* (display_BRIGHTNESS_EN == display_ENABLED) */
}
...
void display_Enable(void) 
{
    ...
    #if (display_BRIGHTNESS_EN == display_ENABLED)
       /* Initialize to zero */
       display_COUNTER_REG = 0x00u;
       display_COMPARE_REG = 0x00u;
       
        /* Enable the PWM for brightness control */
       display_CONTROL_REG = display_CTRL_ENABLE;
    #endif /* (display_BRIGHTNESS_EN == display_ENABLED) */
    
    display_enableVar = display_CTRL_ENABLE;
}
...
void display_Stop(void) 
{
    #if (display_BRIGHTNESS_EN == display_ENABLED)
        /* Disable the PWM */
        display_CONTROL_REG = display_CTRL_DISABLE;
    #endif /* (display_BRIGHTNESS_EN == display_ENABLED) */
    
    display_DmaDisable();
    display_ClearDisplayAll();
    
    display_enableVar  = display_CTRL_DISABLE;
}


/*******************************************************************************
* Function Name: display_SetDisplayRAM
********************************************************************************
*
* Summary:
...
*  RAM directly by writing to display_segLBuffer,
*  display_segMBuffer, display_segHBuffer.
*
* Parameters:
...
void display_SetDisplayRAM(uint8 value, uint8 position) 
{
    ...
    if(position < display_NUM_COM)
    {
        relPosition = position;
        #if(display_SEG_DRIVE == display_HIGH)
            display_segLBuffer[relPosition] = value;
        #else
            display_segLBuffer[relPosition] = ((uint8) ~value);
        #endif /* display_SEG_DRIVE == display_HIGH */
    }    
    else if (position < display_TWO_NUM_COM)
    {
        #if (display_NUM_SEG > 8u)
            relPosition = position - display_NUM_COM;
            #if(display_SEG_DRIVE == display_HIGH)
                display_segMBuffer[relPosition] = value;
            #else
                display_segMBuffer[relPosition] = ((uint8) ~value);
            #endif /* display_SEG_DRIVE == display_HIGH */
        #endif /* (display_NUM_SEG > 8u) */
    }
    else
    {
        #if (display_NUM_SEG > 16u)
            relPosition = position - display_TWO_NUM_COM;
            if(relPosition < display_NUM_COM)
            {
                #if(display_SEG_DRIVE == display_HIGH)
                    display_segHBuffer[relPosition] = value;
                #else
                    display_segMBuffer[relPosition] = ((uint8) ~value);
                #endif /* display_SEG_DRIVE == display_HIGH */
            }
...
void display_SetRC(uint8 row, uint8 column) 
{
    ...
    if(column < display_NUM_COM)
    {
        if(row < 8u)
        {
            rowSeg = row;
            #if(display_SEG_DRIVE == display_HIGH)
                display_segLBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
            #else
                display_segLBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
            #endif /* display_SEG_DRIVE == display_HIGH */
        }
        else if (row < 16u)
        {
                ...
                rowSeg = row - 8u;
                #if(display_SEG_DRIVE == display_HIGH)
                    display_segMBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
                #else
                    display_segMBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
                #endif /* display_SEG_DRIVE == display_HIGH */
            #endif /* (display_NUM_SEG > 8u) */
        }
        else
        {
                ...
                rowSeg = row - 16u;
                #if(display_SEG_DRIVE == display_HIGH)
                    display_segHBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
                #else
                    display_segHBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
                #endif /* display_SEG_DRIVE == display_HIGH */
            #endif /* (display_NUM_SEG > 16u) */
        }
    }
...
void display_ClearRC(uint8 row, uint8 column) 
{
    ...
    if(column < display_NUM_COM)
    {
        if(row < 8u)
        {
            ...
            #if(display_SEG_DRIVE == display_HIGH)
                display_segLBuffer[column] &= ((uint8)(~(uint8)rowSelect));
            #else
                display_segLBuffer[column] |= rowSelect;
            #endif /* display_SEG_DRIVE == display_HIGH */
        }    
        else if (row < 16u)
        {
                ...
                rowSelect = ((uint8)((uint8)0x01u << rowSeg));
                #if(display_SEG_DRIVE == display_HIGH)
                    display_segMBuffer[column] &= ((uint8)(~(uint8)rowSelect));
                #else
                    display_segMBuffer[column] |= rowSelect;
                #endif /* display_SEG_DRIVE == display_HIGH */
            #endif /* (display_NUM_SEG > 8u) */
        }
        else
        {
                ...
                rowSelect = ((uint8)((uint8)0x01u << rowSeg));
                #if(display_SEG_DRIVE == display_HIGH)
                    display_segHBuffer[column] &= ((uint8)(~(uint8)rowSelect));
                #else
                    display_segHBuffer[column] |= rowSelect;
                #endif /* display_SEG_DRIVE == display_HIGH */
            #endif /* (display_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: display_ToggleRC
********************************************************************************
*
* Summary:
...
uint8 display_GetRC(uint8 row, uint8 column) 
{
    ...
    if(column < display_NUM_COM)
    {
        ...
        {
            rowSeg = row;
            rowSegSelect = (uint8)((uint8)display_segLBuffer[column] >> rowSeg);
        }    
        else if (row < 16u)
        {
            #if (display_NUM_SEG > 8u) 
                rowSeg = row - 8u;
                rowSegSelect = (uint8)((uint8)display_segMBuffer[column] >> rowSeg);
            #endif /* (display_NUM_SEG > 8u) */
        }
        else
        {
            #if (display_NUM_SEG > 16u)
                rowSeg = row - 16u;
                rowSegSelect = (uint8)((uint8)display_segHBuffer[column] >> rowSeg);
            #endif /* (display_NUM_SEG > 16u) */
        }
...
/*******************************************************************************
* Function Name: display_ClearDisplay
********************************************************************************
*
* Summary:
...
{  
    ...
    {
        relPosition = position;
        display_segLBuffer[relPosition] = display_CLEAR;
    }    
    else if (position < display_TWO_NUM_COM)
    {
        #if (display_NUM_SEG > 8u)
            relPosition = position - display_NUM_COM;
            display_segMBuffer[relPosition] = display_CLEAR;
        #endif /* (display_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < display_NUM_COM)
            {
                display_segHBuffer[relPosition] = display_CLEAR;
            }
        #endif /* (display_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: display_ClearDisplayAll
********************************************************************************
*
* Summary:
...
void display_ClearDisplayAll(void) 
{
    ...
    for(position = 0u; position < display_NUM_COM; position++)
    {
        display_segLBuffer[position] = display_CLEAR; 
        
        #if (display_NUM_SEG > 8u) 
            display_segMBuffer[position] = display_CLEAR;
        #endif /* (display_NUM_SEG > 8u) */
        
        #if (display_NUM_SEG > 16u)
            display_segHBuffer[position] = display_CLEAR;
        #endif /* (display_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: display_Write7SegNumberDec
********************************************************************************
*
* Summary:
...

{
    ...
    if(position < display_NUM_COM)
    {
        choiceReg = display_SEG_REG_LOW;
        relPosition = position;
    }
    else if (position < display_TWO_NUM_COM)
    {
        choiceReg = display_SEG_REG_MID;
        relPosition = position - display_NUM_COM;
    }
    else
    {
        choiceReg = display_SEG_REG_HIGH;
        relPosition = position - display_TWO_NUM_COM;
    }
    
    /* Routine to extract the sign and to convert to unsigned */
    if(number >= display_ZERO_NEG)
    {
        numberVal = (uint32)number;
        numberSign = display_UNSIGNED;
    }
    else
    {
        numberVal = (uint32)(-number);
        numberSign = display_SIGNED;
    }
    ...
    for(i = digits; i > 0u; i--)
    {
        ...
        bufArray[i-1u] = display_EncodeNumber7Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    
    /* Clean up the array and place sign if needed */
    if(alignment == display_ZERO_PAD)
    {
        if(numberSign == display_SIGNED)
        {
            /* Place sign on the MSB */
            bufArray[0u] = display_EncodeChar7Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != display_ZERO_7CHAR)
            {
                if(numberSign == display_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = display_EncodeChar7Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = display_EncodeChar7Seg('-');
                    }
                }
    ...
    if(alignment == display_LEFT_ALIGN)
    {
        for(i = 0u; i < digits; i++)
        {
            if(bufArray[i] != display_CLEAR)
            {
                if(relPosition < display_NUM_COM)
                {
                    ...
                    switch(choiceReg)
                    {
                        case display_SEG_REG_LOW :
                            display_segLBuffer[relPosition] = bufArray[i];
                            break;
                        case display_SEG_REG_MID :
                            #if (display_NUM_SEG > 8u)
                                display_segMBuffer[relPosition] = bufArray[i];
                            #endif /* (display_NUM_SEG > 8u) */
                            break;
                        case display_SEG_REG_HIGH :
                            #if (display_NUM_SEG > 16u)
                                display_segHBuffer[relPosition] = bufArray[i];
                            #endif /* (display_NUM_SEG > 16u) */
                            break;
                        default :
                            display_segLBuffer[relPosition] = bufArray[i];
    ...
    else
    {
        ...
        for(i = digits; i > 0u; i--)
        {
            if(((i + relPosition) - 1u) < display_NUM_COM)
            {
                /* Decide which row to use */
                switch(choiceReg)
                {
                    case display_SEG_REG_LOW :
                        display_segLBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        break;
                    case display_SEG_REG_MID :
                        #if (display_NUM_SEG > 8u)
                            display_segMBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        #endif /* (display_NUM_SEG > 8u) */
                        break;
                    case display_SEG_REG_HIGH :
                        #if (display_NUM_SEG > 16u)
                            display_segHBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        #endif /* (display_NUM_SEG > 16u) */
                        break;
                    default :
                        display_segLBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
...
/*******************************************************************************
* Function Name: display_Write7SegNumberHex
********************************************************************************
*
* Summary:
...

{
    ...
    for(i = digits; i > 0u; i--)
    {
        selectVal = (uint8)((uint8)numberVal & display_NIBBLE_MASK);
        bufArray[i-1u] = display_EncodeNumber7Seg(selectVal);
        numberVal = (uint32)((uint32)numberVal >> display_SHIFT_NIBBLE);
    }
...
/*******************************************************************************
* Function Name: display_WriteString7Seg
********************************************************************************
*
* Summary:
...

{
    ...
    if(position < display_NUM_COM)
    {
        ...
        for(relPosition = position; relPosition < display_NUM_COM; relPosition++)
        {
            if(relPosition < sizeLimit)
            {
                display_segLBuffer[relPosition] = display_EncodeChar7Seg(character[i]);
                i++;
            }
        }
    }    
    ...
    {
        #if (display_NUM_SEG > 8u) 
            sizeLimit = ((uint8)(strlen(character)) + position) - display_NUM_COM;
            for(relPosition = position - display_NUM_COM;
                relPosition < display_NUM_COM; 
                relPosition++)
            {
                if(relPosition < sizeLimit)
                {
                    display_segMBuffer[relPosition] = display_EncodeChar7Seg(character[i]);
                    i++;
                }
            }
        #endif /* (display_NUM_SEG > 8u) */
    ...
    {
        #if (display_NUM_SEG > 16u)
            sizeLimit = ((uint8)(strlen(character)) + position) - display_TWO_NUM_COM;
            for(relPosition = position - display_TWO_NUM_COM;
                relPosition < display_NUM_COM; 
                relPosition++)
            {
                if(relPosition < sizeLimit)
                {
                    display_segHBuffer[relPosition] = display_EncodeChar7Seg(character[i]);
                    i++;
                }    
            }
        #endif /* (display_NUM_SEG > 16u) */
...
/*******************************************************************************
* Function Name: display_PutChar7Seg
********************************************************************************
*
* Summary:
...

{
    ...
    {
        relPosition = position;
        display_segLBuffer[relPosition] = display_EncodeChar7Seg(character);
    }    
    else if (position < display_TWO_NUM_COM)
    {
        #if (display_NUM_SEG > 8u) 
            relPosition = position - display_NUM_COM;
            display_segMBuffer[relPosition] = display_EncodeChar7Seg(character);
        #endif /* (display_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < display_NUM_COM)
            {
                display_segHBuffer[relPosition] = display_EncodeChar7Seg(character);
            }
        #endif /* (display_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: display_Write7SegDigitDec
********************************************************************************
*
* Summary:
...

{
    ...
    if (digit < 10u)
    {
        ...
        {
            relPosition = position;
            display_segLBuffer[relPosition] = display_EncodeNumber7Seg(digit);
        }    
        else if (position < display_TWO_NUM_COM)
        {
            #if (display_NUM_SEG > 8u)
                relPosition = position - display_NUM_COM;
                display_segMBuffer[relPosition] = display_EncodeNumber7Seg(digit);
            #endif /* (display_NUM_SEG > 8u) */
        } 
        else
        {
            #if (display_NUM_SEG > 16u)
                relPosition = position - display_TWO_NUM_COM;
                if(relPosition < display_TWO_NUM_COM)
                {
                    display_segHBuffer[relPosition] = display_EncodeNumber7Seg(digit);
                }
            #endif /* (display_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: display_Write7SegDigitHex
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: display_Write14SegNumberDec
********************************************************************************
*
* Summary:
...

{
    ...
    {
        selectVal = (uint8)(numberVal % 10u);
        bufArray[i-1u] = display_EncodeNumber14Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    ...
    if(alignment == display_ZERO_PAD)
    {
        ...
        {
            /* Place sign on the MSB */
            bufArray[0u] = display_EncodeChar14Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != display_ZERO_14CHAR)
            {
                if(numberSign == display_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = display_EncodeChar14Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = display_EncodeChar14Seg('-');
                    }
                }
                /* Exit when encountering non-zero number */
                break;
    ...
    if(alignment == display_LEFT_ALIGN)
    {
        for(i = 0u; i < digits; i++)
        {
            if(bufArray[i] != display_CLEAR_16)
            {
                if(relPosition < display_NUM_COM)
                {
                    /* Only show displays that are not empty */
                    display_segLBuffer[relPosition] = LO8(bufArray[i]);
                #if (display_NUM_SEG > 8u)
                    display_segMBuffer[relPosition] = HI8(bufArray[i]);
                #endif /* (display_NUM_SEG > 8u) */
                }
                relPosition++;
            }
    ...
    else
    {
        for(i = digits; i > 0u; i--)
        {
            if(((i + relPosition) - 1u) < display_NUM_COM)
            {
                display_segLBuffer[(i + relPosition) - 1u] = LO8(bufArray[i-1u]);
            #if (display_NUM_SEG > 8u)
                display_segMBuffer[(i + relPosition) - 1u] = HI8(bufArray[i-1u]);
            #endif /* (display_NUM_SEG > 8u) */
            }
        }  
    }
...
/*******************************************************************************
* Function Name: display_Write14SegNumberHex
********************************************************************************
*
* Summary:
*  Displays a hexadecimal number up to 8 characters long, starting at 'position'
...
/*******************************************************************************
* Function Name: display_WriteString14Seg
********************************************************************************
*
* Summary:
...

{
    ...
    for(currPosition = position; currPosition < display_NUM_COM; currPosition++)
    {
        if(currPosition < sizeLimit)
        {
            charDisplay = display_EncodeChar14Seg(character[i]);
            display_segLBuffer[currPosition] = LO8(charDisplay);
            #if (display_NUM_SEG > 8u)
            display_segMBuffer[currPosition] = HI8(charDisplay);
            #endif /* (display_NUM_SEG > 8u) */
            i++;
        }
    }
...
/*******************************************************************************
* Function Name: display_PutChar14Seg
********************************************************************************
*
* Summary:
*  Displays an ascii encoded character at 'position'. This function can display
...

{
    ...
    if(position < display_NUM_COM)
    {
        charDisplay = display_EncodeChar14Seg(character);
        display_segLBuffer[position] = LO8(charDisplay);
    #if (display_NUM_SEG > 8u)
        display_segMBuffer[position] = HI8(charDisplay);
    #endif /* (display_NUM_SEG > 8u) */
    }
}
...
/*******************************************************************************
* Function Name: display_Write14SegDigitDec
********************************************************************************
*
* Summary:
*  Displays a single digit on the specified display. The number in 'digit' 
...

{
    ...
    if((digit < 10u) && (position < display_NUM_COM))
    {
        charDisplay = display_EncodeNumber14Seg(digit);
        display_segLBuffer[position] = LO8(charDisplay);
    #if (display_NUM_SEG > 8u)
        display_segMBuffer[position] = HI8(charDisplay);
    #endif /* (display_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: display_Write14SegDigitHex
********************************************************************************
*
* Summary:
*  Displays a single digit at the specified position. The number in 'digit' 
...
/*******************************************************************************
* Function Name: display_Write16SegNumberDec
********************************************************************************
*
* Summary:
*  Displays a signed integer up to 8 characters long, starting at 'position' 
...

{
    ...
    {
        selectVal = (uint8)(numberVal % 10u);
        bufArray[i-1u] = display_EncodeNumber16Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    ...
    if(alignment == display_ZERO_PAD)
    {
        ...
        {
            /* Place sign on the MSB */
            bufArray[0u] = display_EncodeChar16Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != display_ZERO_16CHAR)
            {
                if(numberSign == display_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = display_EncodeChar16Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = display_EncodeChar16Seg('-');
                    }
                }
                /* Exit when encountering non-zero number */
                break;
...
/*******************************************************************************
* Function Name: display_Write16SegNumberHex
********************************************************************************
*
* Summary:
*  Displays a hexadecimal number up to 8 characters long, starting at 'position'
...
/*******************************************************************************
* Function Name: display_WriteString16Seg
********************************************************************************
*
* Summary:
*  Displays a null terminated string starting at 'position' and ending at either
...

{
    ...
    for(currPosition = position; currPosition < display_NUM_COM; currPosition++)
    {
        if(currPosition < sizeLimit)
        {
            charDisplay = display_EncodeChar16Seg(character[i]);
            display_segLBuffer[currPosition] = LO8(charDisplay);
            #if (display_NUM_SEG > 8u)
                display_segMBuffer[currPosition] = HI8(charDisplay);
            #endif /* (display_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: display_PutChar16Seg
********************************************************************************
*
* Summary:
*  Displays an ascii encoded character at 'position'. This function can display 
...

{
    ...
    if(position < display_NUM_COM)
    {
        charDisplay = display_EncodeChar16Seg(character);
        display_segLBuffer[position] = LO8(charDisplay);
        #if (display_NUM_SEG > 8u)
            display_segMBuffer[position] = HI8(charDisplay);
        #endif /* (display_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: display_Write16SegDigitDec
********************************************************************************
*
* Summary:
*  Displays a single digit on the specified display. The number in 'digit' 
...

{
    ...
    if((digit < 10u) && (position < display_NUM_COM))
    {
        charDisplay = display_EncodeNumber16Seg(digit);
        display_segLBuffer[position] = LO8(charDisplay);
        #if (display_NUM_SEG > 8u)
            display_segMBuffer[position] = HI8(charDisplay);
        #endif /* (display_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: display_Write16SegDigitHex
********************************************************************************
*
* Summary:
*  Displays a single digit at the specified position. The number in 'digit' 
...
/*******************************************************************************
* Function Name: display_PutDecimalPoint
********************************************************************************
*
* Summary:
...

{
    ...
    if(position < display_NUM_COM)
    {
        ...
        {
            #if(display_SEG_DRIVE == display_HIGH)
                display_segLBuffer[relPosition] &= ((uint8)(~(uint8)value));
            #else
                display_segLBuffer[relPosition] |= ((uint8)(~(uint8)value));
            #endif /* (display_SEG_DRIVE == display_HIGH) */
        }
    ...
    else if (position < display_TWO_NUM_COM)
    {
            ...
            {
                #if(display_SEG_DRIVE == display_HIGH)
                    display_segMBuffer[relPosition] &= ((uint8)(~(uint8)value));
                #else
                    display_segMBuffer[relPosition] |= ((uint8)(~(uint8)value));
                #endif /* (display_SEG_DRIVE == display_HIGH) */
            }
    ...
    else
    {
            ...
            if(relPosition < display_NUM_COM)
            {
                ...
                {
                    #if(display_SEG_DRIVE == display_HIGH)
                        display_segHBuffer[relPosition] &= ((uint8)(~(uint8)value));
                    #else
                        display_segHBuffer[relPosition] |= ((uint8)(~(uint8)value));
                    #endif /* (display_SEG_DRIVE == display_HIGH) */
                }
...
/*******************************************************************************
* Function Name: display_GetDecimalPoint
********************************************************************************
*
* Summary:
...
uint8 display_GetDecimalPoint(uint8 position) 
{
    ...
    {
        relPosition = position;
        decPointVal = (uint8)((uint8)(display_segLBuffer[relPosition]) >> display_DEC_POINT);
    } 
    else if (position < display_TWO_NUM_COM)
    {
        #if (display_NUM_SEG > 8u) 
            relPosition = position - display_NUM_COM;
            decPointVal = (uint8)((uint8)(display_segMBuffer[relPosition]) >> display_DEC_POINT);
        #endif /* (display_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < display_NUM_COM)
            {
                decPointVal = (uint8)((uint8)(display_segHBuffer[relPosition]) >> display_DEC_POINT);
            }
        #endif /* (display_NUM_SEG > 16u) */
    }
    
    #if(display_SEG_DRIVE == display_HIGH)
        value = (uint8)(decPointVal & display_DEC_POINT_MASK);
    #else
        value = (uint8)((uint8)((uint8)~decPointVal) & display_DEC_POINT_MASK);
    #endif /* (display_SEG_DRIVE == display_HIGH) */
...
/*******************************************************************************
* Function Name: display_EncodeNumber7Seg
********************************************************************************
*
* Summary:
...
uint8 display_EncodeNumber7Seg(uint8 number) 
{
    ...
    #if(display_SEG_DRIVE == display_HIGH)
        static const uint8 CYCODE display_7SegDigits[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    #endif /*  display_SEG_DRIVE */

    uint8 valNum;
    uint8 valHex;
    
    valNum = (uint8)(number & display_NIBBLE_MASK);
    
    valHex = display_7SegDigits[valNum];
...
/*******************************************************************************
* Function Name: display_EncodeChar7Seg
********************************************************************************
*
* Summary:
...
uint8 display_EncodeChar7Seg(char8 input)  
{
    /* 7-segment ASCII LUT */
    #if(display_SEG_DRIVE == display_HIGH)
        static const uint8 CYCODE display_7SegASCII[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    valLow = (uint8)((uint8)input & display_NIBBLE_MASK);
    
    /* valHigh used to access the row of LUT */
    valHigh = (uint8)((uint8)input >> display_SHIFT_NIBBLE);
    
    /* Check if the high nibble is within valid ASCII range */
    if ((valHigh > display_SEG_ASCII_LIM_LO) && (valHigh < display_SEG_ASCII_LIM_HI))
    {
        valHigh = valHigh - display_SEG_ASCII_OFFSET;
        valHex = display_7SegASCII[valHigh][valLow];
    }
    else
    {
        valHex = display_CLEAR;
    }
...
/*******************************************************************************
* Function Name: display_EncodeNumber14Seg
********************************************************************************
*
* Summary:
...
uint16 display_EncodeNumber14Seg(uint8 number) 
{
        ...
        static const uint8 CYCODE display_14SegDigitsLow[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE display_14SegDigitsHigh[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    valHigh = (uint16)((uint16)display_14SegDigitsHigh[valNum] << display_SHIFT_BYTE);
    valLow = (uint16)display_14SegDigitsLow[valNum];
...
/*******************************************************************************
* Function Name: display_EncodeChar14Seg
********************************************************************************
*
* Summary:
...
uint16 display_EncodeChar14Seg(char8 input) 
{
        ...
        static const uint8 CYCODE display_14SegASCIILow[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE display_14SegASCIIHigh[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    if ((valHigh > display_SEG_ASCII_LIM_LO) && (valHigh < display_SEG_ASCII_LIM_HI))
    {
        ...
        /* Get the 14-seg hex value from high and low byte LUTs */
        valHexHigh = (uint16)((uint16)display_14SegASCIIHigh[valHigh][valLow] << display_SHIFT_BYTE);
        valHexLow = (uint16)display_14SegASCIILow[valHigh][valLow];
        
        valHex = valHexHigh | valHexLow;
    }
    else
    {
        valHex = display_CLEAR_16;
    }
...
/*******************************************************************************
* Function Name: display_EncodeNumber16Seg
********************************************************************************
*
* Summary:
...
uint16 display_EncodeNumber16Seg(uint8 number) 
{
        ...
        static const uint8 CYCODE display_16SegDigitsLow[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE display_16SegDigitsHigh[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    valHigh = (uint16)((uint16)display_16SegDigitsHigh[valNum] << display_SHIFT_BYTE);
    valLow = (uint16)display_16SegDigitsLow[valNum];
...
/*******************************************************************************
* Function Name: display_EncodeChar16Seg
********************************************************************************
*
* Summary:
...
uint16 display_EncodeChar16Seg(char8 input) 
{
        ...
        static const uint8 CYCODE display_16SegASCIILow[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE display_16SegASCIIHigh[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    if ((valHigh > display_SEG_ASCII_LIM_LO) && (valHigh < display_SEG_ASCII_LIM_HI))
    {
        ...
        /* Get the 16-seg hex value from high and low byte LUTs */
        valHexHigh = (uint16)((uint16)display_16SegASCIIHigh[valHigh][valLow] << display_SHIFT_BYTE);
        valHexLow = (uint16)display_16SegASCIILow[valHigh][valLow];
...
/*******************************************************************************
* Function Name: display_SetBrightness
********************************************************************************
*
* Summary:
...
{   
    #if(display_COM_DRIVE == display_HIGH)
        static const uint8 CYCODE display_tempCom[16] = {0x00u, 0x01u, 0x00u, 0x02u, 0x00u, 0x04u,
                                                                  0x00u, 0x08u, 0x00u, 0x10u, 0x00u, 0x20u,
                                                                  0x00u, 0x40u, 0x00u, 0x80u};
    #else
        static const uint8 CYCODE display_tempCom[16] = {0xFFu, 0xFEu, 0xFFu, 0xFDu, 0xFFu, 0xFBu,
    ...
    if(bright == 0u)
    {
        ...
        display_comBuffer[(uint8)((uint8)position << 1u) + 1u] = display_CLEAR;
    }
    else
    {
        /* Restore the commons */
        display_comBuffer[(uint8)((uint8)position << 1u) + 1u]
            = display_tempCom[(uint8)((uint8)position << 1u) + 1u];
    }
    
    /* Update the brightness register */
    display_brightnessReg[position] = bright;
}


/*******************************************************************************
* Function Name: display_GetBrightness
********************************************************************************
*
* Summary:
...
{
    return(display_brightnessReg[position]);
}
