/*******************************************************************************
* File Name: display_ClkInternal.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: display_ClkInternal_Start
********************************************************************************
*
* Summary:
...
void display_ClkInternal_Start(void) 
{
    ...
    display_ClkInternal_CLKEN |= display_ClkInternal_CLKEN_MASK;
	display_ClkInternal_CLKSTBY |= display_ClkInternal_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: display_ClkInternal_Stop
********************************************************************************
*
* Summary:
...
void display_ClkInternal_Stop(void) 
{
    ...
    display_ClkInternal_CLKEN &= (uint8)(~display_ClkInternal_CLKEN_MASK);
	display_ClkInternal_CLKSTBY &= (uint8)(~display_ClkInternal_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: display_ClkInternal_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((display_ClkInternal_CLKEN & display_ClkInternal_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = display_ClkInternal_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = display_ClkInternal_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* display_ClkInternal__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(display_ClkInternal_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(display_ClkInternal_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: display_ClkInternal_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: display_ClkInternal_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = display_ClkInternal_GetSourceRegister();
    uint16 oldDivider = display_ClkInternal_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = display_ClkInternal_CLKEN & display_ClkInternal_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(display_ClkInternal_DIV_PTR, clkDivider);
                display_ClkInternal_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                display_ClkInternal_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(display_ClkInternal_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((display_ClkInternal_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(display_ClkInternal_DIV_PTR, clkDivider);
				display_ClkInternal_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: display_ClkInternal_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(display_ClkInternal_DIV_PTR);
}


/*******************************************************************************
* Function Name: display_ClkInternal_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    display_ClkInternal_MOD_SRC |= modeBitMask & (uint8)display_ClkInternal_MODE_MASK;
}


/*******************************************************************************
* Function Name: display_ClkInternal_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    display_ClkInternal_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(display_ClkInternal_MODE_MASK));
}


/*******************************************************************************
* Function Name: display_ClkInternal_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return display_ClkInternal_MOD_SRC & (uint8)(display_ClkInternal_MODE_MASK);
}


/*******************************************************************************
* Function Name: display_ClkInternal_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = display_ClkInternal_GetDividerRegister();
    uint8 oldSrc = display_ClkInternal_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        display_ClkInternal_MOD_SRC |= CYCLK_SSS;
        display_ClkInternal_MOD_SRC =
            (display_ClkInternal_MOD_SRC & (uint8)(~display_ClkInternal_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: display_ClkInternal_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return display_ClkInternal_MOD_SRC & display_ClkInternal_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: display_ClkInternal_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    display_ClkInternal_PHASE = clkPhase & display_ClkInternal_PHASE_MASK;
}


/*******************************************************************************
* Function Name: display_ClkInternal_GetPhase
********************************************************************************
*
* Summary:
...
{
    return display_ClkInternal_PHASE & display_ClkInternal_PHASE_MASK;
}
