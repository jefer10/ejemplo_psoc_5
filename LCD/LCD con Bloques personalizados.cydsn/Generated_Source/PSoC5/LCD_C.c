...
static void LCD_C_WrDatNib(uint8 nibble) ;
static void LCD_C_WrCntrlNib(uint8 nibble) ;

/* Stores state of component. Indicates whether component is or not
* in enable state.
*/
uint8 LCD_C_enableState = 0u;

uint8 LCD_C_initVar = 0u;


/*******************************************************************************
* Function Name: LCD_C_Init
********************************************************************************
*
* Summary:
...
void LCD_C_Init(void) 
{
    ...
    LCD_C_WrCntrlNib(LCD_C_DISPLAY_8_BIT_INIT);    /* Selects 8-bit mode */
    CyDelay(5u);                                                         /* Delay 5 ms */
    LCD_C_WrCntrlNib(LCD_C_DISPLAY_8_BIT_INIT);    /* Selects 8-bit mode */
    CyDelay(15u);                                                        /* Delay 15 ms */
    LCD_C_WrCntrlNib(LCD_C_DISPLAY_8_BIT_INIT);    /* Selects 8-bit mode */
    CyDelay(1u);                                                         /* Delay 1 ms */
    LCD_C_WrCntrlNib(LCD_C_DISPLAY_4_BIT_INIT);    /* Selects 4-bit mode */
    CyDelay(5u);                                                         /* Delay 5 ms */

    LCD_C_WriteControl(LCD_C_CURSOR_AUTO_INCR_ON);    /* Incr Cursor After Writes */
    LCD_C_WriteControl(LCD_C_DISPLAY_CURSOR_ON);      /* Turn Display, Cursor ON */
    LCD_C_WriteControl(LCD_C_DISPLAY_2_LINES_5x10);   /* 2 Lines by 5x10 Characters */
    LCD_C_WriteControl(LCD_C_DISPLAY_CURSOR_OFF);     /* Turn Display, Cursor OFF */
    LCD_C_WriteControl(LCD_C_CLEAR_DISPLAY);          /* Clear LCD Screen */
    LCD_C_WriteControl(LCD_C_DISPLAY_ON_CURSOR_OFF);  /* Turn Display ON, Cursor OFF */
    LCD_C_WriteControl(LCD_C_RESET_CURSOR_POSITION);  /* Set Cursor to 0,0 */
    CyDelay(5u);

    #if(LCD_C_CUSTOM_CHAR_SET != LCD_C_NONE)
        LCD_C_LoadCustomFonts(LCD_C_customFonts);
    #endif /* LCD_C_CUSTOM_CHAR_SET != LCD_C_NONE */
}


/*******************************************************************************
* Function Name: LCD_C_Enable
********************************************************************************
*
* Summary:
...
*  LCD_C_Init() turns on the LCD.
*
*******************************************************************************/
void LCD_C_Enable(void) 
{
    LCD_C_DisplayOn();
    LCD_C_enableState = 1u;
}


/*******************************************************************************
* Function Name: LCD_C_Start
********************************************************************************
*
* Summary:
*  Performs initialization required for the components normal work.
...
void LCD_C_Stop(void) 
{
    /* Calls LCD Off Macro */
    LCD_C_DisplayOff();
    LCD_C_enableState = 0u;
}


/*******************************************************************************
*  Function Name: LCD_C_Position
********************************************************************************
*
* Summary:
...
void LCD_C_Position(uint8 row, uint8 column) 
{
    ...
    {
        case (uint8)0:
            LCD_C_WriteControl(LCD_C_ROW_0_START + column);
            break;
        case (uint8) 1:
            LCD_C_WriteControl(LCD_C_ROW_1_START + column);
            break;
        case (uint8) 2:
            LCD_C_WriteControl(LCD_C_ROW_2_START + column);
            break;
        case (uint8) 3:
            LCD_C_WriteControl(LCD_C_ROW_3_START + column);
            break;
        default:
...
/*******************************************************************************
* Function Name: LCD_C_PrintString
********************************************************************************
*
* Summary:
...
{
    LCD_C_WriteData((uint8)character);
}


/*******************************************************************************
*  Function Name: LCD_C_WriteData
********************************************************************************
*
* Summary:
...
void LCD_C_WriteData(uint8 dByte) 
{
    ...
    LCD_C_IsReady();
    nibble = dByte >> LCD_C_NIBBLE_SHIFT;

    /* Write high nibble */
    LCD_C_WrDatNib(nibble);

    nibble = dByte & LCD_C_NIBBLE_MASK;
    /* Write low nibble */
    LCD_C_WrDatNib(nibble);
}
...
/*******************************************************************************
*  Function Name: LCD_C_WriteControl
********************************************************************************
*
* Summary:
...
void LCD_C_WriteControl(uint8 cByte) 
{
    ...
    nibble = cByte >> LCD_C_NIBBLE_SHIFT;
    
    /* WrCntrlNib(High Nibble) */
    LCD_C_WrCntrlNib(nibble);
    nibble = cByte & LCD_C_NIBBLE_MASK;
...
void LCD_C_IsReady(void) 
{
    uint8 value;
    uint32 timeout;
    timeout = LCD_C_READY_DELAY;

    /* Clear LCD port*/
    LCD_C_PORT_DR_REG &= ((uint8)(~LCD_C_PORT_MASK));
    ...
    */
        ...
        value = LCD_C_PORT_PC_REG & ((uint32) (~ LCD_C_DM_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_C_PORT_PC_REG = value | LCD_C_HIGH_Z_DATA_DM;
        ...
        /* Mask off data pins to clear old values out */
        value = LCD_C_PORT_DM0_REG & ((uint8)(~LCD_C_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_C_PORT_DM0_REG = value | (LCD_C_HIGH_Z_DM0 & LCD_C_DATA_MASK);

        /* Mask off data pins to clear old values out */
        value = LCD_C_PORT_DM1_REG & ((uint8)(~LCD_C_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_C_PORT_DM1_REG = value;

        /* Mask off data pins to clear old values out */
        value = LCD_C_PORT_DM2_REG & ((uint8)(~LCD_C_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_C_PORT_DM2_REG = value;
    
    #endif /* CY_PSOC4 */
    
    /* Make sure RS is low */
    LCD_C_PORT_DR_REG &= ((uint8)(~LCD_C_RS));

    /* Set R/W high to read */
    LCD_C_PORT_DR_REG |= LCD_C_RW;
    ...
    LCD_C_PORT_DR_REG &= ((uint8)(~LCD_C_RW));
    ...
    LCD_C_PORT_DR_REG &= ((uint8)(~LCD_C_PORT_MASK));
        ...
        value = LCD_C_PORT_PC_REG & ((uint32)(~ LCD_C_DM_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_C_PORT_PC_REG = value | LCD_C_STRONG_DATA_DM;
        ...
        */
        LCD_C_PORT_DM0_REG &= ((uint8)(~LCD_C_DATA_MASK));
        /* Mask off data pins to clear high z values out */
        value = LCD_C_PORT_DM1_REG & ((uint8)(~LCD_C_DATA_MASK));
        /* Configure data pins to Strong Drive, others unchanged */
        LCD_C_PORT_DM1_REG = value | (LCD_C_STRONG_DM1 & LCD_C_DATA_MASK);

        /* Mask off data pins to clear high z values out */
        value = LCD_C_PORT_DM2_REG & ((uint8)(~LCD_C_DATA_MASK));
        /* Configure data pins to Strong Drive, others unchanged */
        LCD_C_PORT_DM2_REG = value | (LCD_C_STRONG_DM2 & LCD_C_DATA_MASK);
...
static void LCD_C_WrDatNib(uint8 nibble) 
{
    ...
    /* Clear data pins */
    LCD_C_PORT_DR_REG &= ((uint8)(~LCD_C_DATA_MASK));

    /* Write data, bring E high */
    #if(0u != LCD_C_PORT_SHIFT) /* MISRA forbids shift by 0 so need to handle that */
        LCD_C_PORT_DR_REG |= 
            (LCD_C_E | ((uint8)(((uint8) nibble) << LCD_C_PORT_SHIFT)));
    #else
        LCD_C_PORT_DR_REG |= (LCD_C_E | nibble);
    #endif /* (0u != LCD_C_PORT_SHIFT) */
...
/*******************************************************************************
*  Function Name: LCD_C_WrCntrlNib
********************************************************************************
*
* Summary:
...
static void LCD_C_WrCntrlNib(uint8 nibble) 
{
    ...
    LCD_C_PORT_DR_REG &= ((uint8)(~(LCD_C_RS | LCD_C_RW)));
    ...
    *******************************************************************************/
    ...
    {
        static char8 const CYCODE LCD_C_hex[16u] = "0123456789ABCDEF";
        
        LCD_C_PutChar((char8) LCD_C_hex[value >> LCD_C_BYTE_UPPER_NIBBLE_SHIFT]);
        LCD_C_PutChar((char8) LCD_C_hex[value & LCD_C_BYTE_LOWER_NIBBLE_MASK]);
    }


    /*******************************************************************************
    *  Function Name: LCD_C_PrintInt16
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        LCD_C_PrintInt8((uint8)(value >> LCD_C_U16_UPPER_BYTE_SHIFT));
        LCD_C_PrintInt8((uint8)(value & LCD_C_U16_LOWER_BYTE_MASK));
    }

    
    /*******************************************************************************
    *  Function Name: LCD_C_PrintInt32
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        uint8 shift = LCD_C_32_BIT_SHIFT;
        ...
        while (shift != 0u)
        {
            ...
            */
            shift -= LCD_C_8_BIT_SHIFT;
            
            /* Print 8 bits of uint32 hex number */
            LCD_C_PrintInt8((uint8) ((uint32) (value >> shift)));
        }
    }
    
    
    /*******************************************************************************
    *  Function Name: LCD_C_PrintNumber
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    *  Function Name: LCD_C_PrintU32Number
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        uint8 tmpDigit;
        char8 number[LCD_C_NUMBER_OF_REMAINDERS_U32 + 1u];
        uint8 digIndex = LCD_C_NUMBER_OF_REMAINDERS_U32;
        ...
        */
        ...
        while(value >= LCD_C_TEN)
        {
            /* Extract decimal digit, indexed by 'digIndex', from 'value' and
            * convert it to ASCII character.
            */
            tmpDigit = (uint8) (((uint8) (value % LCD_C_TEN)) + (uint8) LCD_C_ZERO_CHAR_ASCII);
        ...
        */
        ...
        /* Print out number */
        LCD_C_PrintString(&number[digIndex]);
    }
